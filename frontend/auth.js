...
/**
 * @param loginIfNotAuthenticated {boolean} indicates whether the user should
 * authenticate himself is he isn't yet.
 * @returns {Promise<string>}
 */
export async function authorize(loginIfNotAuthenticated) {
...
async function startAuthorizationCodeFlow(clientId, redirectUri, authorizationEndpoint, scope) {
  ...
  window.location.href = `${authorizationEndpoint.protocol}//${authorizationEndpoint.host}${authorizationEndpoint.pathname}?${authParams.toString()}`;
...
export async function getUserInfo(userInfoEndpoint) {
  ...
  if (response.status >= 500) {
    throw `failed to fetch user info (server error): ${response.status} ${response.statusText}: ${await response.text()}`;
  } else if (response.status >= 400) {
    throw `failed to fetch user info: ${response.status} ${response.statusText}: ${await response.text()}`;
  }
